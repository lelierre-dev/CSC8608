...
def main() -> None:
    ...
    out_path = os.path.join(base_dir, "outputs", "baseline.png")
    save(img, out_path)
...
def run_text2img_experiments() -> None:
    ...
    seed = 42
    prompt = (
        "ultra-realistic product photo of a premium red wine bottle with a gold-foil "
        "label and a crown-shaped gold seal on a white background, studio lighting, soft shadow, very sharp"
    )
    negative = "text, watermark, logo, low quality, blurry, deformed"
    ...
    for name, scheduler_name, steps, guidance in plan:
        ...
        img = out.images[0]
        out_path = os.path.join(base_dir, "outputs", f"t2i_{name}.png")
        save(img, out_path)
        print(
            "T2I",
            name,
...
def run_img2img_experiments() -> None:
    ...
    for name, strength in strengths:
        ...
        img = out.images[0]
        out_path = os.path.join(base_dir, "outputs", f"i2i_{name}.png")
        save(img, out_path)
        print(
            "I2I",
            name,
