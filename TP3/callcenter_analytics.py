...
# Regex "strict" email (fonctionne apres normalisation)
EMAIL_RE = re.compile(r"\b[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}\b")

# Telephone : 7+ digits au total, separateurs optionnels (espaces, -, .)
...
def preclean(text: str) -> str:
    ...
    # Separer chiffres colles a des mots : "5550199thank" -> "5550199 thank"
    t = re.sub(r"(\d)([a-z])", r"\1 \2", t)
    t = re.sub(r"([a-z])(\d)", r"\1 \2", t)

    # Ajouter un espace apres ponctuation collee entre deux tokens : "hello.thank" -> "hello. thank"
    t = re.sub(r"([a-z0-9])([.,!?])([a-z0-9])", r"\1\2 \3", t)

    # Apostrophes genantes pour les heuristiques (ex: "don't" -> "dont")
...
def normalize_spelled_tokens(text: str) -> str:
    ...
    - collage des sequences de digits separes (>= 6 digits)
    """
    t = preclean(text)

    # Normalisation email parlee
    t = re.sub(r"\bdot\b", ".", t)
    t = re.sub(r"\bat\b", "@", t)
    ...
    # Coller les digits isoles : "5 5 5 0 1 9 9" -> "5550199"
    def _collapse(m):
    ...
    # Re-separer digits/lettres au cas ou apres collapse
    t = re.sub(r"(\d)([a-z])", r"\1 \2", t)
    return t
...
def redact_order_id(text: str) -> Tuple[str, int]:
    """
    Masque un identifiant apres 'order number is' meme s'il est epele (a.x.1.9.7.3.5).
    """
    count = 0
...
def redact_spoken_email(text: str) -> Tuple[str, int]:
    """
    1) masque les vrais emails detectables (apres normalisation)
    2) sinon masque par contexte : "reach me ..." jusqu'a un marqueur (also/phone/order/thank) ou fin
    """
    count = 0
    ...
    # (2) fallback par contexte (robuste meme si ASR "massacre" le local-part)
    ctx = re.compile(
...
def redact_pii(text: str) -> Tuple[str, Dict[str, int]]:
    ...
    - normalise tokens eples (digits, dot/at)
    - masque order id (contexte)
    - masque email (standard ou contexte)
    - masque telephone (sequences de digits)
    """
...
def score_intents(text: str) -> Dict[str, int]:
    ...
    for intent, kws in INTENTS.items():
        s = 0
        for kw in kws:
            # TODO: compter occurrences naivement
            s += t.count(kw)
        scores[intent] = s
    return scores
...
def pick_intent(scores: Dict[str, int]) -> str:
    # intention avec meilleur score ; fallback si tous a 0
