...
# Avertissement bbox très petite
...
if do_segment:
	...
	st.session_state["last_img_stem"] = img_path.stem

# 5) Sauvegarde overlay (hors bouton Segmenter)
if "last_overlay" in st.session_state:
	save = st.button("Sauvegarder overlay")
	if save:
		OUT_DIR.mkdir(parents=True, exist_ok=True)
		out_path = OUT_DIR / f"overlay_{st.session_state['last_img_stem']}.png"
		cv2.imwrite(
			str(out_path),
			cv2.cvtColor(st.session_state["last_overlay"], cv2.COLOR_RGB2BGR),
		)
